...
const notUndefinedSetValue = (prop, expr) => {
...
export const toJSON = {
  ...
  long(args) {
    ...
    const prop = args.field.name;
    return notUndefinedSetValue(prop, t.callExpression(t.memberExpression(t.logicalExpression('||', t.memberExpression(t.identifier('message'), t.identifier(prop)), getDefaultTSTypeFromProtoType(args.field, args.isOptional)), t.identifier('toString')), []));
  },
  ...
  type(args) {
    const prop = args.field.name;
    const name = args.context.getTypeName(args.field); // TODO isn't the nested conditional a waste? (using ts-proto as reference)
    // maybe null is OK?
  ...
  bytes(args) {
    ...
    if (args.isOptional) {
      ...
      // message.queryData !== undefined && (obj.queryData = base64FromBytes(message.queryData !== undefined ? message.queryData : new Uint8Array()));
      expr = t.callExpression(t.identifier('base64FromBytes'), [t.conditionalExpression(t.binaryExpression('!==', t.memberExpression(t.identifier('message'), t.identifier(args.field.name)), t.identifier('undefined')), t.memberExpression(t.identifier('message'), t.identifier(args.field.name)), getDefaultTSTypeFromProtoType(args.field, args.isOptional))]);
    }
...
export const arrayTypes = {
  ...
  // }
  long(args) {
    return t.callExpression(t.memberExpression(t.logicalExpression('||', t.identifier('e'), getDefaultTSTypeFromProtoType(_objectSpread(_objectSpread({}, args.field), {}, {
  ...
  bytes(args) {
    args.context.addUtil('base64FromBytes');
    return t.callExpression(t.identifier('base64FromBytes'), [t.conditionalExpression(t.binaryExpression('!==', t.identifier('e'), t.identifier('undefined')), t.identifier('e'), getDefaultTSTypeFromProtoType(_objectSpread(_objectSpread({}, args.field), {}, {
      rule: undefined // so it's treated as type not an array...
