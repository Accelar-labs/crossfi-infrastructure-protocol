...
export const aminoInterface = {
  defaultType(args) {
    return t.tsPropertySignature(t.identifier(args.context.aminoCaseField(args.field)), t.tsTypeAnnotation(getTSTypeFromProtoType(args.field.type)));
  },
  ...
  type({
    ...
    const properties = protoFieldsToArray(Type).map(field => {
      ...
      const isOptional = getFieldOptionality(field, isOneOf); // TODO how to handle isOptional from parent to child...
  ...
  typeArray({
    ...
    const parentField = field;
    const Type = context.getTypeFromCurrentPath(field, currentProtoPath); // TODO how to handle isOptional from parent to child... 
  ...
  array(args) {
    ...
    //     )
    // );
    return t.tsPropertySignature(t.identifier(args.context.aminoCaseField(args.field)), t.tsTypeAnnotation(arrayTypeNDim(getTSTypeFromProtoType(args.field.type), 1)));
  }
