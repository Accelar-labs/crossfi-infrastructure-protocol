...
export const fromAminoJsonMethod = ({
  ...
  const ctxs = fields.map(({
    ctx
  }) => ctx);
  ctxs.forEach(ctx => {// console.log('imports, ', ctx.imports)
  });
  return arrowFunctionExpression([fromAminoParams], t.blockStatement([t.returnStatement(t.objectExpression(fields.map(({
    field
  }) => field)))]), t.tsTypeAnnotation(t.tsTypeReference(t.identifier(proto.name))));
};