...
var createHelperObject = function createHelperObject(_ref7) {
  ...
  mutations = mutations.map(function (mutation) {
    return _objectSpread(_objectSpread({}, mutation), {}, {
      methodName: (0, _case.camel)(mutation.methodName)
    });
  });
  return t.exportNamedDeclaration(t.variableDeclaration('const', [t.variableDeclarator(t.identifier(name), t.objectExpression([t.objectProperty(t.identifier('encoded'), t.objectExpression(mutations.map(function (mutation) {
  ...
  }))), t.objectProperty(t.identifier('toJSON'), t.objectExpression(mutations.map(function (mutation) {
    return addToJSONMethod(mutation);
  }))), t.objectProperty(t.identifier('fromJSON'), t.objectExpression(mutations.map(function (mutation) {
    return addFromJSONMethod(mutation);
  }))), t.objectProperty(t.identifier('fromPartial'), t.objectExpression(mutations.map(function (mutation) {
